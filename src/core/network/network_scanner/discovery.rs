use crate::core::network::network_scanner::types::MDNS_SERVICES;
...
pub async fn discover_mdns_devices(
    ...
    for service_name in MDNS_SERVICES {
        let mdns_stream = match mdns::discover::all(service_name, timeout_duration) {
            Ok(stream) => stream,
            Err(_) => continue,
        };
                                ...
                                trust_dns_proto::rr::RecordType::A => {
                                    if let Some(rdata) = record.data {
                                        if let Some(a_record) = rdata.as_a() {
                                            device_ip = Some(IpAddr::V4(a_record.0));
                                        }
                                    }
                                }
                                trust_dns_proto::rr::RecordType::AAAA => {
                                    if let Some(rdata) = record.data {
                                        if let Some(aaaa_record) = rdata.as_aaaa() {
                                            device_ip = Some(IpAddr::V6(aaaa_record.0));
                                        }
                                    }
                                }
                                _ => {}
