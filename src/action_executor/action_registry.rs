...
use crate::action_executor::ActionEventHandler;
use crate::action_executor::{types::ComponentState, ActionExecutor, ActionFn};
...
pub trait ActionRegistry {
    ...
    fn register_action(
        ...
        name: &str,
        action_fn: Rc<
            dyn Fn(&mut ActionExecutor, &ActionEventHandler, &ActionContext) -> Result<(), String>,
        >,
    );
}
...
impl ActionRegistry for ActionExecutor {
    ...
    fn register_content_actions(&mut self) {
        self.register_action(
            "update",
            Rc::new(
                |executor: &mut ActionExecutor, handler: &ActionEventHandler, _| {
                    let target = handler.target.as_ref().ok_or("no target specified")?;
    ...
    fn register_data_actions(&mut self) {
        self.register_action(
            "submit",
            Rc::new(|executor, handler, context| executor.handle_submit(handler, context)),
        );

        self.register_action(
            "collect",
            Rc::new(|executor, handler, context| executor.handle_collect(handler, context)),
        );

        self.register_action(
            "validate",
            Rc::new(|executor, handler, context| executor.handle_validate(handler, context)),
        );
    }
}
